...
이미지 생성 실행·모델 선택·비용 고지는 gil-creative:higgsfield-core 계약에 위임하고, 이 스킬은
...
- **이미지 생성 실행·모델 선택·크레딧 고지 안 함** — `gil-creative:higgsfield-core` 계약에 위임한다(§4 생성 실행).
...
조립한 프롬프트와 제약을 `gil-creative:higgsfield-core` 계약에 넘긴다. 모델 선택·라이브 파라미터 조회·비용 프리플라이트·폴링은 전부 그쪽이 한다.

마스코트나 특정 인물이 **여러 컷에 일관되게** 나와야 하면 먼저 `gil-creative:higgsfield-identity`로 참조를 만든다(Soul / Element 판정 포함).
...
- 생성 위임: gil-creative:higgsfield-core
- 결과 URL: [생성 결과]
- 정합 점검: [통과 | 어긋난 항목과 조치]
```
...
| 스킬 | 시점 |
|---|---|
| `gil-creative:higgsfield-core` | 위임: 생성 실행·비용·모델 |
| `gil-creative:higgsfield-identity` | 선행: 마스코트·모델 일관성 참조 |
| `gil-creative:higgsfield-product` | 인접: 제품 중심 비주얼 10모드 |
| `gil-creative:design-brand-system` | 선행: 브랜드 디자인 시스템 정의 |
| `gil-creative:design-tokens-transformer` | 선행: 토큰 파이프라인 |
| `gil-creative:design-slop-check` | 후속: AI 슬롭 점검 |
...
- 생성 계약·비용·모델 선택: `gil-creative:higgsfield-core` 라이브 계약.
