...
가설-설계-측정 실험 프레임과 성장 루프(추천/바이럴/무료도구/공동마케팅) 전술로 실험 설계서를 만들고, 서술형 부분은 gil:ai-slop-reviewer → gil:humanize-korean으로 다듬습니다.
[책임 경계] vs gil-creative:campaign-planner: 이 스킬=개별 실험 설계와 성장 루프 전술(가설·A/B·ICE·바이럴 계수), 저 스킬=목표·채널·예산·KPI를 묶은 전체 캠페인 기획. 실험은 캠페인의 하위 단위입니다.
...
| 전체 캠페인 기획이 필요한 경우 | 본 스킬 범위 밖. `gil-creative:campaign-planner`로 안내 |

## 후처리 (필수 체인 — 서술형 부분만)

가설 설명·성장 루프 설계 서술·파트너십 제안 등 **서술형(prose) 산출물**은 발행 전 후처리 체인을 거칩니다. 샘플 크기 계산·ICE 점수표·지표 위계 표 등 **수치·표 산출물은 대상이 아닙니다**.

**권장 체인(서술형 부분)**: `growth-experiment(서술형) → gil:ai-slop-reviewer → gil:humanize-korean → 최종 검수(◆최종본, humanize Phase 6)`

## 관련 스킬

| 스킬 | 사용 시점 |
|------|----------|
| `gil-creative:campaign-planner` | 실험을 목표·채널·예산·KPI 캠페인으로 확장 (페어 분리) |
| `gil-creative:landing-page-conversion-audit` | A/B 대상 랜딩 페이지의 전환 저해 요소 진단 |
| `gil-creative:performance-report` | 실험 결과·성과 분석 보고서 |
| `gil-creative:email-sequence` | 무료 도구·추천 이후 너처링·초대 이메일 시퀀스 |
| `gil:ai-slop-reviewer` | 사후 체이닝(필수, 서술형 부분): 실험 설계 서술의 AI 패턴 검수 |
| `gil:humanize-korean` | 사후 체이닝(필수, 서술형 부분): ai-slop-reviewer 다음 단계로 한국어 AI 티 제거 |

## 이 스킬을 사용하지 말아야 할 때

- 목표·채널·예산·KPI를 묶은 전체 캠페인 기획: `gil-creative:campaign-planner` 사용
- 랜딩 페이지 전환 요소 진단: `gil-creative:landing-page-conversion-audit` 사용
- 성과 분석 보고서: `gil-creative:performance-report` 사용
