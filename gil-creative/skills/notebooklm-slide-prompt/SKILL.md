...
요약: **NotebookLM에 넣을 준비물(소스·대본·구조·이미지 프롬프트)을 만드는 빌더**가 이 스킬입니다. 실제 `.pptx` 파일이 필요하면 `gil:pptx-designer`를 호출하세요. "PPT 파일로 뽑아줘"는 `gil:pptx-designer`, "NotebookLM 슬라이드 프롬프트 만들어줘"는 이 스킬입니다.
...
- Google AI Studio
- Vertex AI
- (GIL 사용자) `gil-creative:higgsfield-image` 스킬에 그대로 전달
