...
| 6 | **이미지 프롬프트 생성** | `gil-creative:creative-architect` (+ `gil-creative:gpt-image-2-prompt`·`gil-creative:gemini-3-image-prompt`) |
| 7 | 이미지 생성 (외부, BYOK) | `gil-creative:image-bridge` |
| 8 | **포맷 빌더** | 포맷별(§4) |
...
| 단계 | 도구 |
|---|---|
| AI 슬롭 제거 → 언어별 윤문 | `gil:ai-slop-reviewer` → `gil:humanize-korean` |
| 한국어 맞춤법 | `gil:korean-spell-check` |
