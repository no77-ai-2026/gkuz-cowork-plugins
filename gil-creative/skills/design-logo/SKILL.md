...
- **이미지 생성 실행·모델 선택·비용 고지 안 함** — `gil-creative:higgsfield-core` 계약에 위임한다(§5 생성 위임). 모델은 recraft_v4_1 벡터 계열이지만, 라이브 파라미터 조회·비용·폴링은 전부 위임처가 한다.
...
조립한 프롬프트와 제약을 `gil-creative:higgsfield-core` 계약에 넘긴다. 모델(recraft_v4_1) 선택·라이브 파라미터 조회(`models_explore`)·비용 프리플라이트(`get_cost`)·폴링(`job_status`)은 전부 그쪽이 한다.
...
- 생성 위임: gil-creative:higgsfield-core (recraft_v4_1)
- 결과 URL: [생성 결과들]
- 정합 점검: [통과 | 어긋난 항목과 조치]
...
| `gil-creative:higgsfield-image` | 인접: recraft 프롬프트 크래프트 SSOT |
| `gil-creative:design-brand-visual` | 형제: 히어로·OG·목업 비주얼 |
