...
1. `gil-creative:pixel-audit` — 픽셀·전환 추적 점검
2. `gil-creative:meta-ads-manager` — 캠페인 구조·소재 운영
3. `gil-creative:meta-ads-analyzer` — 성과 분석·개선안
4. `gil-creative:performance-report` — 리포트 정리 → `gil:ai-slop-reviewer`
...
- 모든 고객 대상 콘텐츠는 `gil:ai-slop-reviewer`(필수) → `gil:humanize-korean`.
