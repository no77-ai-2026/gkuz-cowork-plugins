...
[책임 경계] vs gil-creative:notebooklm-slide-prompt: 이 스킬=지금 바로 열리는 .pptx 파일, 저 스킬=NotebookLM에 넣을 슬라이드 생성 프롬프트.
...
3. 색 일관성 (한 팔레트 안에서만)
4. AI 슬롭 카피 검출 (`gil-creative:design-slop-check` 체이닝)
5. 청중·톤 적합성 (사람 검수)
...
| 스킬 | 사용 시점 |
|---|---|
| `gil:docx-generator` | DOCX 문서 생성 (같은 Claude 톤 시스템 공유) |
| `gil:hwpx-writer` | 한컴 한글 문서 |
| `gil:xlsx-creator` | 엑셀 데이터 시트 (차트 데이터 소스) |
| `gil:pdf-writer` | 다국어 PDF 변환 |
| `gil-creative:copywriting` | 슬라이드 카피 작성 |
| `gil:humanize-korean` | 한국어 카피 자연화 |
| `gil-creative:design-slop-check` | 슬라이드 카피 AI 슬롭 검수 |
| `gil-creative:design-prompt-builder` | Claude Design에 동시 시안 요청 시 |
| `gil-creative:gemini-3-image-prompt` | 슬라이드 일러스트·배경 이미지 프롬프트 |
