...
- **gil:close-management**: 월말·급여 결산 업무
...
> 분산 분석 요약표·KPI 대시보드 같은 표·숫자 산출물 자체는 체인 대상이 아니며, 엑셀화가 필요하면 `gil:xlsx-creator`, 시각화가 필요하면 `gil:data-visualizer`로 라우팅한다. 체인은 표를 둘러싼 해설·권고·요약 문장에만 적용한다.

## 이 스킬을 사용하지 말아야 할 때

- **재무제표 공식 작성**: K-IFRS 기준 재무제표 작성은 `gil:financial-statements` 스킬을 사용하세요.
- **월말 결산·급여 정산**: 회계 마감 업무는 `gil:close-management` 스킬이 더 적합합니다.
- **세금 계산**: 세무 관련 문의는 `gil:tax-helper` 스킬을 사용하세요.
