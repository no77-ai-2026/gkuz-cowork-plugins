...
[책임 경계] vs gil:market-analyst: 이 스킬=학술 논문과 연구 리서치, 저 스킬=시장 조사와 비즈니스 리서치.
...
관찰 데이터(회계·재무·경제·경영) 실증연구라면 문헌검토와 함께 **Kill-fast 원칙**을 먼저 적용하세요. 대량 수집·집필 전에 (1)데이터 취득가능성, (2)예비 신호검정(부호·유의·기업고정효과 견고성), (3)효과 선례를 확인해 죽는 주제를 빨리 걸러냅니다. 관심변수가 실제 어느 문서·데이터에 있는지 **소표본 feasibility**를 대량수집보다 먼저 확인하세요. 상세 SOP는 `gil:research-methodology`의 `archival-empirical-playbook.md`, 강건성·파이프라인은 `gil:research-analysis`의 `causal-robustness-sop.md`·`data-pipeline-traps.md`를 참조합니다.

## 관련 스킬

- **gil:curriculum-designer**: 커리큘럼 설계, 학습 목표 정의
- **gil:assessment-creator**: 시험·평가 문제 출제

## 이 스킬을 사용하지 말아야 할 때

- **커리큘럼 및 강의 설계** → gil:curriculum-designer 스킬이 더 적합합니다
- **시험 문제 출제** → gil:assessment-creator 스킬을 사용하세요
- **시장 조사·비즈니스 리서치** → gil:market-analyst 스킬이 더 적합합니다
