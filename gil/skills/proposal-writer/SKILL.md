...
주의: 정부 지원사업 신청서는 `gil:kr-gov-grant`를 사용하세요. 본 스킬은 **B2B 영업 고객 대상** 제안서만 다룹니다.
투자자 IR 자료는 `gil:investor-relations`를 사용하세요.
...
1. `gil:ai-slop-reviewer`로 본문 검수
2. `gil:docx-generator`(워드/PDF) 또는 `gil:pptx-designer`(슬라이드) 출력
3. 견적서를 분리 발행하려면, 본 스킬이 가격 섹션(10번)을 기반으로 항목·수량·단가·소계·VAT·합계 표(견적 명세)를 직접 작성한 뒤 `gil:xlsx-creator`로 스프레드시트 출력하세요
...
## 🔍 다음 단계
1. gil:ai-slop-reviewer로 본문 검수
2. gil:docx-generator로 PDF 출력
3. 견적서 분리 발행 시 — 가격 섹션 기반 견적 명세 표 작성 후 gil:xlsx-creator로 출력
```
...
- **NDA 정보 주의**: 고객사 NDA 대상 정보 포함 시 `gil:nda-triage` 체이닝 권고
...
**Before (입력 prep)**:
- `gil:market-analyst` — 고객사 산업·시장 분석 자료 prep

**After (출력 후처리)**:
- `gil:ai-slop-reviewer` — 본문 AI slop 검수 (필수)
- `gil:docx-generator` — Word/PDF 출력
- `gil:pptx-designer` — 슬라이드 출력
- `gil:xlsx-creator` — 견적서 별도 발행 (본 스킬이 가격 섹션 기반 견적 명세 표를 작성한 뒤 스프레드시트로 출력)

**Alternative (대체 스킬)**:
- `gil:investor-relations` — 투자자 대상 IR (B2B 고객 아님)
- `gil:kr-gov-grant` — 정부 지원사업 신청서 (B2B 영업 아님)

## 관련 커맨드

- `/harness` — 본 스킬 자체의 추가 개선·재생성
- 등록된 스킬 체인:
  - B2B 영업 제안서: `gil:market-analyst → gil:proposal-writer → gil:ai-slop-reviewer → gil:docx-generator`
  - B2B 영업 슬라이드: `gil:market-analyst → gil:proposal-writer → gil:ai-slop-reviewer → gil:pptx-designer`
  - 견적서 분리: `gil:proposal-writer → gil:xlsx-creator` (proposal-writer가 가격 섹션 기반 견적 명세 표를 직접 작성)
