...
> KPI/OKR 달성률 표·평가 등급 매트릭스·성과 대시보드 같은 **숫자·표 산출물**은 위 텍스트 체인 대상이 아닙니다. 표·대시보드가 필요하면 `gil:xlsx-creator` 또는 `gil:data-visualizer`로 라우팅하세요. 혼합 산출물은 **서술 부분만** 체인을 적용하고 표는 데이터 라우팅을 유지합니다.
...
- **채용·온보딩 프로세스**: JD 작성, 면접 설계, 신입 온보딩은 `gil:employment-manager` 스킬이 더 적합합니다.
- **오퍼 레터·연봉 협상 문서**: `gil:draft-offer` 스킬을 사용하세요.
- **원격 근무 정책 수립**: 재택 정책, 협업 도구 선택은 `gil:people-operations` 스킬을 사용하세요.