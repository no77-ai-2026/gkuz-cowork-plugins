...
- **before**: `gil:investor-relations` — IR 자료 작성 시 KRX 공식 시세 필요
- **before**: DART MCP (`gil-business/.mcp.json`) — 공시·재무 분석과 결합
- **after**: `gil:variance-analysis` — 시세 변동 분석
- **after**: `gil:xlsx-creator` — 시세 데이터 엑셀화
- **after**: `gil:executive-summary` — 경영진 1pager에 시세 요약
