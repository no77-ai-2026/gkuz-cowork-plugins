...
- **정책충격 DiD**: 기존 원가/성과 패널 재사용 + 처치 레이어만 신규 → 강건성은 `gil:research-analysis`의 `causal-robustness-sop.md`.
...
- **무결성 게이트 연계** — 초안·분석 산출 직후 `gil:devil-review`의 AI 연구 무결성 게이트 + 인용 충실성 점검을 통과시킨다.
...
research-methodology  (본 스킬: 방법론 설계·무결성 원칙)
  → gil:research-analysis  (통계·데이터 분석; UZ 데이터는 uz-research-data 참조)
  → gil:paper-writer  (본문 집필)
  → gil:devil-review  (무결성 게이트·인용 충실성 점검)
  → gil:journal-selection / journal-style-adapter  (투고)
```

UZ 현지 데이터·설문은 `gil:public-data`(stat.uz 등)와, 정책·법령은 `gil-legal`과 연계. 트릴링구얼 설문은 `references/uz-field-context.md`.
