...
- "임원 주간보고", "팀 주간보고"
[책임 경계] 제품 발견·로드맵/마일스톤 기획은 gil:roadmap-manager, 본 스킬은 운영 케이던스(주간보고)
...
- **gil:ai-slop-reviewer**: AI 패턴·격식체 톤 검수 (텍스트 산출물 필수)
- **gil:humanize-korean**: 한국어 자연스러움 보정 (ai-slop-reviewer 다음 단계)
- **gil:executive-summary**: 1pager를 더 압축된 C-level 보고로 변환
- **gil:pptx-designer**: 슬라이드 발표 자료로 출력
...
- **NDA 정보 주의**: 외부 공유용 보고서 작성 시 회사 비밀 정보 자동 검사 (`gil:nda-triage` 권고)
...
- `gil:humanize-korean` — 한국어 자연스러움 보정 (ai-slop-reviewer 다음)
- `gil:executive-summary` — C-level 1pager로 더 압축
- `gil:pptx-designer` — 발표 슬라이드 출력

**Alternative**:
- `gil:daily-briefing` — 일간 보고 (주간이 아닌 일일)
