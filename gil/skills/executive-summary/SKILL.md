...
기본 출력은 gil:html-report로 단일 HTML(이미지·CSS·JS 인라인, 카톡·이메일 바로 공유)이며, pdf/docx/pptx/hwpx 변환은 옵션 체이닝. 입력 가능: gil-creative:performance-report 출력 · gil:financial-statements · gil:variance-analysis · gil:weekly-report · 외부 보고서. 한국 임원/이사회 표준 What/So What/Now What + K-IFRS 재무 지표 우선.
[책임 경계] vs gil-creative:performance-report: executive-summary=임원 압축 요약(≤500단어), performance-report=마케팅 풀 리포트(전체).
...
본 스킬의 **기본 출력은 마크다운 + `gil:html-report` 렌더링한 단일 HTML 파일**입니다.
...
- 주간/월간 운영 보고 (`gil:weekly-report` 출력)
- 외부 분석 (시장조사·컨설팅 보고서)
...
→ gil:variance-analysis 결과 입력
→ executive-summary가 K-IFRS 지표 우선 + What/So What/Now What 마크다운 생성
→ gil:html-report (mode=status)로 단일 HTML 렌더링
...
**Before (입력 prep)**:
- `gil:financial-statements` — 재무제표
- `gil:variance-analysis` — 변동분석
- `gil:weekly-report` — 주간보고
- (외부 보고서)

**Renderer (기본 출력 = 단일 HTML)**:
- `gil:html-report` — **기본 렌더러**, mode=status (이미지·CSS·JS 인라인, 카톡 공유 가능)

**Converter (선택 변환 분기)**:
- `gil:pdf-writer` — 인쇄·결재용 PDF
- `gil:docx-generator` — 편집 가능한 .docx
- `gil:pptx-designer` — 이사회 슬라이드 1매
- `gil:hwpx-writer` — 한국 공공기관 .hwpx

**Post-process**:
- `gil/ai-slop-reviewer` — 격식체·정량 출처 검수
- `gil:humanize-korean` — 한국어 자연스러움 보강

**Alternative**:
- `gil:weekly-report` — 팀 단위 주간 (1pager 아닌 6섹션)
- `gil:strategy-planner` — 전략 문서 (요약 아닌 본문)
