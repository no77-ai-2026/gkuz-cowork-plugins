...
10. AI 슬롭 표현 없음 (혁신적인·차세대·재정의하는 등 — `gil-creative:design-slop-check` 체이닝 권장)
...
한컴오피스 없는 환경에서 공문서가 필요한 경우 DOCX로 대체 가능합니다. 한컴 변환이 필요한 경우 `gil:hwpx-writer` 스킬 참조.
...
| 스킬 | 사용 시점 |
|---|---|
| `gil:pptx-designer` | 발표용 슬라이드 생성 (같은 Claude 톤 디자인 시스템 공유) |
| `gil:hwpx-writer` | 한컴 한글 문서 생성 (HWPX) |
| `gil:xlsx-creator` | 엑셀 데이터 시트 |
| `gil:pdf-writer` | PDF 변환·다국어 PDF |
| `gil:humanize-korean` | 카피 AI 슬롭 자연화 |
| `gil:ai-slop-reviewer` | 텍스트 산출물 슬롭 검수 |
| `gil-creative:design-slop-check` | Claude Design 톤과 일관성 검수 |
