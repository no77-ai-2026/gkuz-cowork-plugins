...
- **gil:strategy-planner**: 전략 기획서 작성 시 활용
- **gil:market-analyst**: 시장 분석이 필요한 경우 먼저 실행
- **gil:pptx-designer**: 브리프를 PPT로 변환
- **gil:ai-slop-reviewer → gil:humanize-korean**: 최종 산출물 AI 패턴 검수 후 한국어 표현을 자연스럽게 다듬습니다 (필수)

## 연계 스킬 (v2.2.0 추가)

- 현황 진단 전에 문제 정의가 모호하면 `gil:problem-solving`으로 SCQ·이슈트리를 먼저 세운다.
- 브리프에 인용하는 시장·경쟁 수치는 `gil:research-verify`로 출처 병기·검증 후 반영한다.
