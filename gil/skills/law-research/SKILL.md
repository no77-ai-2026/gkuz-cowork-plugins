...
- **gil:compliance-check**: 규제 준수 점검·ESG 보고 (현행 법령 근거는 본 스킬에서 조회 후 연계)
- **gil:legal-risk**: 법적 리스크 분석·IP 전략·법령 변화 영향 분석 (현행 원문·개정 추적·행위시법은 본 스킬에서 연계)
- **gil:contract-review**: 계약서 검토·이용약관 작성 (계약서에 인용된 조문 검증은 본 스킬 `verify_citations`)
- **gil:patent-search** / **gil:patent-analyzer**: 특허·상표·디자인 (KIPRIS, 본 스킬과 데이터 소스 상이)
- **gil:iros-registry-automation**: 대법원 등기부등본 (iros, 본 스킬과 데이터 소스 상이)
- **gil:mfds-safety**: 식약처 규제 (식약처 소스, 본 스킬과 데이터 소스 상이)
- **gil:nda-triage**: NDA 비밀유지계약 전문 검토
- **gil:ai-slop-reviewer** / **gil:humanize-korean**: 서술형 산출물 후처리 체인 (법령 본문·표는 제외)

## 이 스킬을 사용하지 말아야 할 때 (책임 분담)

- **특허·상표·디자인 검색·분석** → `gil:patent-search` / `gil:patent-analyzer` (KIPRIS 소스)
- **계약서·이용약관·개인정보처리방침 작성·검토** → `gil:contract-review` (다만 계약서 인용 조문 검증은 본 스킬)
- **규제 준수·내부 감사·ESG 보고** → `gil:compliance-check` (현행 법령 근거 조회만 본 스킬 연계)
- **법적 리스크 평가·IP 포트폴리오 전략·법령 변화 영향 분석** → `gil:legal-risk` (현행 원문·개정 추적·행위시법은 본 스킬 연계)
- **대법원 등기부등본 발급·조회** → `gil:iros-registry-automation`
- **식약처 규제·안전 검사** → `gil:mfds-safety`
- **NDA 비밀유지계약 검토** → `gil:nda-triage`
