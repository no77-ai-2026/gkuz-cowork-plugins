...
> 주간 CS 요약 보고서의 **수치·KPI 표**(핵심 지표·유형별 현황표 등)는 후처리 체인 대상이 아닙니다. 표·대시보드 형태가 필요하면 `gil:xlsx-creator` 또는 `gil:data-visualizer`로 라우팅하고, 보고서의 **서술 부분(Top 3 이슈·인사이트·액션 권고)만** 위 체인으로 다듬습니다.

## 이 스킬을 사용하지 말아야 할 때

- **초기 티켓 분류 및 긴급도 판단** → gil:ticket-triage 스킬이 더 적합합니다
- **일반 고객 응답 초안 작성** → gil:draft-response 스킬을 사용하세요
- **지식베이스 문서 작성** → gil:kb-article 스킬을 사용하세요
