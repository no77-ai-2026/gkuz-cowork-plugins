...
[책임 경계] vs gil:research-assistant: 이 스킬=개인 학습용 최신 정보 빠른 조사, 저 스킬=학술 논문용 문헌 검토와 인용 관리.
...
- **gil:learning-project**: 마일스톤 핵심 질문이 이 스킬의 입력이 된다
- **gil:learning-material**: 이 스킬의 종합본을 도식·HTML 학습자료로 변환

## 이 스킬을 사용하지 말아야 할 때

- **학술 논문용 문헌 검토·인용 관리** → `gil:research-assistant`
- **시장·비즈니스 조사** → `gil:market-analyst`
- **깊은 다중 소스 교차검증 리포트(단일 질문, 비학습)** → 번들 `/deep-research` 워크플로우

## 연계 스킬 (v2.2.0 추가)

- 학습·기술 조사가 아닌 비즈니스 분석용 리서치(출처 병기·검증 루프·조건부 결론)는 `gil:research-verify`를 사용한다.
