...
- "시각화해줘"
- "대시보드 만들어줘"
Mermaid·Recharts·Chart.js·Tremor·ECharts 중 적합한 스택으로 인터랙티브 대시보드를 제작하고, gil:pptx-designer / gil:docx-generator로 PPT·Word 변환까지 이어집니다.
...
2. 데이터 테이블 + 차트 이미지 → `gil:pptx-designer`로 전달
3. PPT 슬라이드 생성

Word 보고서 요청 시:
1. 차트 이미지 + 분석 텍스트 → `gil:docx-generator`로 전달
2. Word 문서 생성 (차트 임베드)
...
3. 데이터 구조 확인 → 부적합 시 `gil:data-explorer` 권유
4. 차트 산출 후 사용자 검토 요청

## 이 스킬을 사용하지 말아야 할 때
- **데이터 탐색/프로파일링** → `gil:data-explorer` 사용
- **공공데이터 조회** → `gil:public-data` 사용
- **랜딩 페이지 내 차트 섹션** → `gil-creative:landing-page`에서 Q4 효과 선택 시 통합 산출
- **상세페이지 내 가격 비교 차트** → `gil-commerce:product-detail`에서 직접 처리

## 흡수 방법론 (knowledge-work-plugins@2cf4294, Apache-2.0)

- 출처: `data/create-viz + data/data-visualization`
- 출판 품질 차트 가이드(차트 유형 선택 트리·컬러 접근성·라벨링 규칙, matplotlib/seaborn/plotly) 확장. 다중 뷰 대시보드는 gil:build-dashboard로 위임(경계).
