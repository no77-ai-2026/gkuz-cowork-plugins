...
```
vendor-manager → gil:ai-slop-reviewer → gil:humanize-korean → 최종 검수(◆최종본, humanize Phase 6)
```

- `gil:ai-slop-reviewer` — 서술형 본문의 AI 티 패턴을 검수·수정합니다.
- `gil:humanize-korean` — 검수된 본문을 자연스러운 한국어 보고체로 다듬습니다.

벤더 평가표, 리스크 평가 매트릭스 등 **점수·표 산출물**은 산문 후처리 대상이 아니며 `gil:xlsx-creator` 또는 `gil:data-visualizer`로 라우팅합니다.

## 이 스킬을 사용하지 말아야 할 때

- **실제 계약서 법적 검토** → gil:contract-review 스킬을 사용하세요
