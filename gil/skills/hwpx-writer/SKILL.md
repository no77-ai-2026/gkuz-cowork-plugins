...
[책임 경계] vs gil:docx-generator: 이 스킬=한컴 .hwpx 한글 파일, 저 스킬=MS 워드 .docx 파일.
...
HWPX 파일은 한컴오피스에서 최적으로 열립니다. 한컴오피스가 설치되지 않은 환경에서는 `gil:docx-generator` 스킬 사용을 권장합니다.
...
- `gil:docx-generator` - DOCX(Word) 문서 생성
- `gil:pptx-designer` - 발표용 PPT 슬라이드 생성
- `gil:xlsx-creator` - 엑셀 데이터 시트 생성
