...
PDF 파일이 필요하면 생성한 HTML을 gil:pdf-writer로 넘겨 디자인 그대로 PDF로 변환하세요 (weasyprint를 직접 설치·호출하지 말 것).
...
`gil:html-report`는 cowork 텍스트 산출 파이프라인의 **터미널 렌더러**입니다.
`gil:executive-summary`, `gil:financial-statements`, `gil:sbiz365-analyst` 등이 생성한 마크다운 보고서를 **단일 파일·자체 완결형(self-contained) HTML**로 변환합니다.
...
| 모드 | 구조 섹션 | 대상 스킬 |
|------|-----------|-----------|
| **`status`** | 메트릭 카드 4개 · 하이라이트 · 완료 테이블 · Velocity SVG 막대 차트 · Carryover | `gil:executive-summary`, `gil:daily-briefing` |
| **`incident`** | TL;DR 다크 배너 · 타임라인 · 로그 발췌 `<details>` · 코드 diff 패널 · 영향 테이블 · 액션 체크리스트 | `gil:compliance-check` |
| **`plan`** | 요약 KPI 스트립 · 마일스톤 수직 타임라인 · 데이터 플로우 SVG · 슬라이스 테이블 · 리스크 그리드 · 성공 지표 | `gil:sbiz365-analyst` |
| **`explainer`** | 사이드 네비 · `<details>` 접이식 단계 · 탭 코드 블록(vanilla JS) · FAQ 아코디언 · 콜아웃 박스 | `gil:*`, `gil:*` |
| **`financial`** | KPI 카드 4개 · 손익계산서 테이블(항목/당기/전기/증감/증감률) · Variance SVG 수평 막대 차트 · 주석 패널 | `gil:financial-statements` |
| **`pr`** | TL;DR · PR 메타 행(파일수·+/−·브랜치) · Before/After 2단 카드 · 파일 투어 `<details>` · 핵심 포인트 · 테스트 체크리스트 · 롤아웃 단계 | `gil:investor-relations` |
...
```
[텍스트 스킬] → gil:ai-slop-reviewer → gil:humanize-korean → gil:html-report (서식 선택)
```

최소 체인 (빠른 렌더링):
```
[텍스트 스킬] → gil:html-report (서식 선택)
```
...
> `design_system` 지정 시 `gil-creative:design-system-library`에서 토큰을 자동 로드합니다 — 별도 선행 스킬 호출 불필요.
...
- 슬라이드는 `gil:pptx-designer`, 독립 차트는 `gil:data-visualizer`가 맡습니다.
...
|-------------|-----------|-----------|-------------|--------|
| `gil:executive-summary` | `status` | [`references/integration-tests/executive-summary-input.md`](references/integration-tests/executive-summary-input.md) | [`references/integration-tests/executive-summary-rendered.html`](references/integration-tests/executive-summary-rendered.html) | ★★★★☆ (4/5) |
| `gil:financial-statements` | `financial` | [`references/integration-tests/financial-statements-input.md`](references/integration-tests/financial-statements-input.md) | [`references/integration-tests/financial-statements-rendered.html`](references/integration-tests/financial-statements-rendered.html) | ★★★★☆ (4/5) |
| `gil:sbiz365-analyst` | `plan` | [`references/integration-tests/sbiz365-analyst-input.md`](references/integration-tests/sbiz365-analyst-input.md) | [`references/integration-tests/sbiz365-analyst-rendered.html`](references/integration-tests/sbiz365-analyst-rendered.html) | ★★★★☆ (4/5) |
| `gil:daily-briefing` | `status` (daily variant) | [`references/integration-tests/daily-briefing-input.md`](references/integration-tests/daily-briefing-input.md) | [`references/integration-tests/daily-briefing-rendered.html`](references/integration-tests/daily-briefing-rendered.html) | ★★★★☆ (4/5) |
...
선택지:
1. **기본 — Wanted 디자인 시스템** (권장): `gil-creative:design-system-library`의 `systems/wanted.md` 토큰 적용 (Pretendard·블루 프라이머리·화이트 캔버스·한국형 비즈니스 무드)
2. **다른 브랜드 시스템**: `gil-creative:design-system-library` 76종에서 지정 (예: claude·clickhouse·clay·stripe·notion)
