...
### 선행 스킬 (입력 생성)
- `gil-creative:blog` — 블로그 포스트 생성
- `gil-creative:landing-page` — 랜딩 페이지 카피라이팅
- `gil-creative:newsletter` — 뉴스레터 작성
- `gil-creative:copywriting` — 마케팅 카피 작성
- `gil:docx-generator` — DOCX 문서 생성
- `gil:pptx-designer` — PPTX 발표자료 생성

### 후속 스킬 (선택적)
- `gil-creative:gemini-3-image-prompt` — (선택) 블로그 발행 체인의 이미지 생성 단계
