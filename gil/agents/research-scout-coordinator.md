...
- 원고 텍스트는 `gil:ai-slop-reviewer`(필수) → `gil:humanize-korean`.

## 신규 라우팅 (v2.2.0)

- 논문·특허가 아닌 비즈니스 리서치(시장·경쟁·규제 사실 수집)는 `gil:research-verify`로 라우팅해 출처 등급·[미검증] 태그·Red 반론 루프를 적용한다.
- 조사 결과를 의사결정 이슈로 구조화할 때는 `gil:problem-solving`으로 이어준다.
