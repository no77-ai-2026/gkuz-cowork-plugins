...
페어 gil-commerce:product-detail과 명확히 구분 — 본 스킬은 카피, 페어는 코드 (shadcn/ui React 또는 HTML).
...
- `gil-creative:copywriting` — 일반 광고 카피
- `gil-commerce:product-detail` — shadcn/ui 기반 웹 상세페이지
- `gil:ai-slop-reviewer` — 텍스트 검수 (이 스킬에서 자동 호출)

## 이 스킬을 사용하지 말아야 할 때

- 랜딩 페이지(마케팅 원페이지): `gil-creative:landing-page` 사용
- 블로그 포스팅: `gil-creative:blog` 사용
- SNS 콘텐츠: `gil-creative:sns-content` 사용
