...
매크로 전략 모드(채널 믹스·3단계 가격·시즌 프로모션 캘린더·재구매 자동화·KPI 대시보드 references 제공) + 통합 1장 모드(선행 산출물 종합) 2계층으로 동작하며, 전략 1장 직후 gil:ai-slop-reviewer를 자동 체이닝합니다.
[책임 경계] 본 스킬은 이커머스 셀러 즉시 실행 전술 + 채널 전략. 중장기 사업 전략은 gil:strategy-planner, 운영 자동화 진단은 gil-commerce:commerce-automation-audit 사용.
...
- `gil:strategy-planner` — 중장기 사업 전략 (페어, 상위 레벨)

## 이 스킬을 사용하지 말아야 할 때

- **중장기 사업 전략 (분기·연간)**: `gil:strategy-planner` 사용
- **운영 자동화 진단 풀세트**: `gil-commerce:commerce-automation-audit` 사용
- **IR 덱·투자 유치 전략**: `gil:pptx-designer` + 별도 사업기획 스킬 사용
...
한국 이커머스 시즌(설·가정의달·추석·빼빼로·블프·연말), 프로모션 유형, 시즌별 채널 우선순위, 광고 집중 시점은 `references/promotion.md` 참조. 프로모션 카피는 `gil-commerce:commerce-channel-message`로 작성.
...
> [책임 경계] 자동화 진단은 `gil-commerce:commerce-automation-audit`가 SSOT로 풀세트 진행. 본 스킬은 매출/채널 전략에 자동화 측면을 요약 통합할 때만 사용 — 상세 자동화 설계는 commerce-automation-audit로 위임.
