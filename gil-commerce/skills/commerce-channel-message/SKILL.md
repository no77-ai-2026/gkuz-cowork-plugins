...
3개 동작 모드(채널 분기 메시지 15종 / 운영 카피 / 앱 푸시 기획)가 자연어로 자동 선택되며, 6 심리 방아쇠 + 채널별 심리 상태 매트릭스 + 인지 편향 9종을 적용하고, 텍스트 산출물 직후 gil:ai-slop-reviewer를 자동 체이닝합니다.
[책임 경계] 페어 gil-domain-copywriting(도메인 비특정 단일 목적 카피)·gil-creative:copywriting(이커머스 외 범용)과 구분 — 본 스킬은 이커머스 채널 운영 메시지 전용. 상세페이지 카피는 gil-commerce:detail-page-copy, 발송 전 법규 게이트는 gil-commerce:commerce-marketing-compliance-kr.
...
- `gil-creative:sns-content` — SNS 콘텐츠 단독 심화 작업
- `gil:ai-slop-reviewer` — AI 패턴 검수 (자동 체이닝)

## 이 스킬을 사용하지 말아야 할 때

- **도메인 비특정 단일 목적 카피**: `gil-domain-copywriting` 사용
- **이커머스 외 일반 광고 카피**: `gil-creative:copywriting` 사용
- **상세페이지 13섹션 카피**: `detail-page-copy` 사용
- **이메일 뉴스레터 본문**: `gil-creative:newsletter` 또는 `gil-creative:email-sequence` 사용
- **광고 캠페인 전체 기획**: `gil-creative:campaign-planner` 사용
...
푸시 기획 후속: 발송 직전 `gil-commerce:commerce-marketing-compliance-kr`로 정통망법 게이트(야간 21:00-08:00 광고 발송 금지·수신거부·발신자 정보)를 통과시킵니다.
...
| **쿠팡** | 검색영역 + 비검색영역 동시 | 즉시 구매 | 검색(전환 메시지) vs 비검색(노출 메시지) 분리 | CPM 167배 차이 → 영역별 메시지 다르게. gil-commerce:marketplace-coupang-ads와 연계 |

### 인지 편향 9종 적용 가이드

페어 스킬 `gil-creative:campaign-planner`와 동일한 9종 (Kahneman 행동경제학 표준 카탈로그 기반).
