...
- 카피·키워드는 `gil-creative:copywriting`(러·우즈벡 트릴링구얼)로 산출 후 현지 검수 1회 필수.
- 상세 이미지는 `gil-commerce:detail-page-image`(러시아어 우선) 또는 `gil-creative:gemini-3-image-prompt`.
...
- CPC 대략 200~2,000 UZS. 캠페인 기획은 `gil-creative:campaign-planner`(uz-marketing-channels) 연동.
...
- 리뷰·Q&A는 러시아어 중심 응대.
- 리뷰 분석·VOC 우선순위 → `gil-commerce:commerce-voc-triage`.
...
```
gil-commerce:detail-page-copy / gil-commerce:product-detail  (상세페이지)
  → gil-commerce:detail-page-image / gil-creative:gemini-3-image-prompt  (이미지, 러시아어 우선)
  → marketplace-uzum  (본 스킬: 등록·운영)
  → gil-creative:campaign-planner  (광고)
  → gil-commerce:commerce-voc-triage  (리뷰·VOC)
  → gil:data-explorer  (매출·전환 분석)
```
